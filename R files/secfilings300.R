...
info <- GroupBy(trades, c(ID, Symbol, Time),
                SharesTraded = Sum(if (difference < 0) Size else 0),
                  TotalVolume = Sum(Size),
                  
                  OrderBy(MidTime = asc(abs(difference)), inputs = c(MidPrice = Price, MidSize = Size), limit = 1),
...
info <- info [ FirstPrice < MidPrice && MidPrice > LastPrice]

info <- OrderBy(info, dsc((LastPrice), limit = 200000)

##info <- OrderBy(info, PortionSold = dsc(SharesTraded / TotalVolume))
