library(gtBase)
library(nanex)
library(methods)

data <- Read(nanex_trades)

market <- ReadCSV("/tmp/hourly_market_index.csv", c(,), header = TRUE, sep = "|")

agg <- Segmenter(
GroupBy(data,
group=c(SymbolHour = MsOfDay %/% 3600000 + 24 * (Date$GetYear() * 366 + Date$GetDayOfYear()-1),Symbol),
OrderBy(LastTime = dsc(MsOfDay), inputs = Price, outputs = LastPrice, limit = 1),
OrderBy(FirstTime = asc(MsOfDay), inputs = Price, outputs = FirstPrice, limit = 1),
Count = Count(),
Total = Sum(Size)
));

agg <- Generate(agg, symbolchange = (FirstPrice - LastPrice) / FirstPrice)


datajoin <- Join( agg, SymbolHour,market, Hour)

#datajoin <- OrderBy(datajoin, dsc(Count), limit = 200000)

#View(datajoin)
final <- Generate(datajoin, index= symbolchange - marketchange)
 
charts <- GroupBy(data, group = Symbol,
                  chart = LineChart(inputs = c(SymbolHour, index), length = 24),
                  cnt = Count())[cnt > 100]

final <- OrderBy(final, asc(Hour), limit=200000);


#find max and min subtract min from all 

#length= max of realigned plus 1